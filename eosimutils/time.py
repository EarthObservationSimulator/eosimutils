...
class AbsoluteDateArray:
    ...
    def linspace(
        ...
        """
        Construct an AbsoluteDateArray with uniformly spaced times.

        The returned array always includes the start time. Subsequent values
        are generated by incrementing by ``step`` until the next value would
        exceed the stop time.

        Args:
            start (AbsoluteDate): Start time.
            stop (AbsoluteDate): Stop time.
            step (float): Step size in seconds.

        Returns:
            AbsoluteDateArray: Uniformly spaced AbsoluteDateArray.

        Raises:
            ValueError: If step is not positive.
            ValueError: If start is greater than stop.
        """
        start_et = start.to_spice_ephemeris_time()
        stop_et = stop.to_spice_ephemeris_time()
        ...
        ephemeris_time = ephemeris_time[ephemeris_time <= stop_et]

        return AbsoluteDateArray(ephemeris_time)

    def window(
        self,
        start: AbsoluteDate = None,
        stop: AbsoluteDate = None,
    ) -> "AbsoluteDateArray":
        """
        Window the AbsoluteDateArray to a specified interval.

        The returned array is guaranteed to include the specified start and stop
        times as endpoints. If the start or stop time does not already exist in
        the array, it is inserted.

        Args:
            start (AbsoluteDate, optional): Start time of the window.
                                            Defaults to the first time in the array.
            stop (AbsoluteDate, optional): Stop time of the window.
                                        Defaults to the last time in the array.

        Returns:
            AbsoluteDateArray: Windowed AbsoluteDateArray.

        Raises:
            ValueError: If start or stop are outside the bounds of the array.
            ValueError: If start is greater than stop.
        """
        if self.length == 0:
            return AbsoluteDateArray(np.array([], dtype=float))

        array_start = self.ephemeris_time[0]
        array_stop = self.ephemeris_time[-1]

        start_et = (
            array_start
            if start is None
            else start.to_spice_ephemeris_time()
        )
        stop_et = (
            array_stop
            if stop is None
            else stop.to_spice_ephemeris_time()
        )

        if start_et < array_start or start_et > array_stop:
            raise ValueError("start is outside the bounds of the array")

        if stop_et < array_start or stop_et > array_stop:
            raise ValueError("stop is outside the bounds of the array")

        if start_et > stop_et:
            raise ValueError("start must be less than or equal to stop")

        mask = (self.ephemeris_time >= start_et) & (
            self.ephemeris_time <= stop_et
        )

        windowed = self.ephemeris_time[mask]

        if len(windowed) == 0 or windowed[0] != start_et:
            windowed = np.insert(windowed, 0, start_et)

        if windowed[-1] != stop_et:
            windowed = np.append(windowed, stop_et)

        return AbsoluteDateArray(windowed)
