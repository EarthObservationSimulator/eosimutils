...
def convert_frame(positions: np.ndarray, velocities: np.ndarray, times: np.ndarray,
    from_frame: ReferenceFrame, to_frame: ReferenceFrame) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    for i, t in enumerate(times):
        ...
        new_positions[i] = t_matrix[0:3,0:3].dot(positions[i]) + \
                           t_matrix[0:3,3:6].dot(velocities[i])
        new_velocities[i] = t_matrix[3:6,0:3].dot(positions[i]) + \
                            t_matrix[3:6,3:6].dot(velocities[i])
...
class Trajectory(Timeseries):
    ...
    Attributes:
        time (AbsoluteDates): Contains a vector of time samples.
    ...
    """
    def __init__(self, time: "AbsoluteDates", data: list, frame: ReferenceFrame):
        if len(data) != 2 or data[0].shape[1] != 3 or data[1].shape[1] != 3:
            raise ValueError("Data must be a list containing two arrays: pos (Nx3) and vel (Nx3).")
    ...
    def resample(self, new_time: np.ndarray, method: str = "linear") -> "Trajectory":
    ...
    def _arithmetic_op(self, other, op, interp_method: str = "linear") -> "Trajectory":
        """
        ...
        if np.isscalar(other):
            ...
            return Trajectory(AbsoluteDates(self.time.et.copy()), new_data, self.frame)
            ...
            if self.frame != other.frame:
                pos_conv, vel_conv = \
                convert_frame(self.data[0], self.data[1], self.time.et, self.frame, other.frame)
            ...
            return Trajectory(AbsoluteDates(self.time.et.copy()), new_data, result_frame)
        else:
    ...
    def constant_position(cls, t1: float, t2: float,
        ...
        time_obj = AbsoluteDates(np.array([t1, t2]))
        traj = np.column_stack((np.tile(position, (2, 1)),
                                np.zeros((2, 3))))
        return cls(time_obj, [traj[:, :3], traj[:, 3:]], frame)

    @classmethod
    def constant_velocity(cls, t1: float, t2: float, velocity: np.ndarray,
                          initial_position: np.ndarray,
                          frame: ReferenceFrame) -> "Trajectory":
        """
        Create a constant velocity trajectory.
        """
        if velocity.shape != (3,) or initial_position.shape != (3,):
            raise ValueError("velocity and initial_position must be 3-element arrays.")
        dt = t2 - t1
        final_position = initial_position + velocity * dt
        time_obj = AbsoluteDates(np.array([t1, t2]))
        traj = np.column_stack((np.vstack((initial_position, final_position)),
